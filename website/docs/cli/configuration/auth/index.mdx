...
auth:
  ...
  integrations:
    <integration-name>:            # Named integration configurations
      kind: <integration-kind>     # aws/ecr, aws/eks
      via:
...
<DocCardList items={[
  ...
  {type: 'link', href: '/cli/commands/auth/ecr-login', label: 'atmos auth ecr-login', description: 'Login to AWS ECR registries'},
  {type: 'link', href: '/cli/commands/auth/eks-token', label: 'atmos auth eks-token', description: 'Generate EKS bearer tokens for kubectl'},
]} />
